...
class TestCaseAccount(unittest.TestCase):
    ...
    def test205_disable_address(self):
        ...
        try:
            response = Service.Service().disable_coin_address(DB.DB().get_address(354558, 1)[0])
            Common.search_str(str(response), [self.SUCCESS_RESULT])
            print(response)
        except Exception as ex:
            raise ex
    ...
    def test207_get_recharge_record(self):
        ...
        try:
            response = Service.Service().get_recharge_recorde(58, '2018-01-01 00:00:00', '2018-08-14 00:00:00', 1, 100)
            Common.search_str(str(response), [self.SUCCESS_RESULT])
            print(response)
        except Exception as ex:
            raise ex
    ...
    def test208_get_withdraw_record(self):
        ...
        try:
            response = Service.Service().get_withdraw_recorde(58, '2018-01-01 00:00:00', '2018-08-14 00:00:00', 1, 100)
            Common.search_str(str(response), [self.SUCCESS_RESULT])
            print(response)
        except Exception as ex:
            raise ex
    ...
    def test209_apply_withdraw(self):
        ...
        try:
            response = Service.Service().apply_withdraw(1, 6264, 0.001, 0.01, 0, Common.generate_otp('HZTERXEO7C2655JP'),
                                                        ComMeth.ComMeth().get_code_noali(1, 13866666666), 1, Common.pwdTwo)
            Common.search_str(str(response), [self.SUCCESS_RESULT])
            print(response)
        except Exception as ex:
            raise ex
    ...
    def test210_cancel_withdraw(self):
        ...
        try:
            response = Service.Service().cancel_withdraw(1, DB.DB().get_withdraw_record('out_coin_transaction', 354558, 1, 1)[0])
            Common.search_str(str(response), [self.SUCCESS_RESULT])
            print(response)
        except Exception as ex:
            raise ex
